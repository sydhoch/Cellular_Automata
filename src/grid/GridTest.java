...
import static Enums.Shape.*;
import static Enums.Arrangement.*;
...
class GridTest {
    ...
    @Test
    void testReadFile() {
        Grid myGrid = new Grid("gol-grid-3.csv", COMPLETE, RECTANGLE, TOROIDAL);
    ...
    void checkStateMapCorrectAtBeginning() {
        ...
        Grid myGrid = new Grid("perc-grid-1.csv", COMPLETE, RECTANGLE, TOROIDAL);
    ...
    void compareLists(List<Integer[]> expected, Grid myGrid) {
        for(int a = 0; a < 3; a++) {
            //List<Integer[]> actual = myGrid.getCellsInState(a); //fix
